...
sum(is.na(hotel_b_m2$child_comp))
...
* Odds of fulfillment are smaller by a factor of .18 when the deposit is refundable, .001 when the deposit is not refundable compare to when there is not deposit, 
...
* The odds of fulfillment  decreased by 99% when there is a non refundable deposit, by 81% when there is a refundable deposit compared to when there is no deposit
...
# Create ROC curve
roc(actual_response, predicted_response, plot = TRUE)
...
The hotel booking has many other interesting variables, and they could also be consider to build another model
